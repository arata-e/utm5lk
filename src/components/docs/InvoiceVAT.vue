<template>
  <div >
        <div v-if="INVOICES_VAT">
          <b-table hover small v-if="INVOICES_VAT" sticky-header striped id="tableStatistics"
          head-variant="light"
          :items="INVOICES_VAT"
          :fields="fields"
...
export default {
  data () {
    return {
      fields: [
        ...
        { key: 'full_name', label: this.$i18n.t('Full Name'), sortable: true, sortDirection: 'desc' },
        { key: 'kpp', label: this.$i18n.t('KPP'), sortable: true, sortDirection: 'desc' },
  ...
  methods: {
    DownloadPDF (row) {
      let params = {docType: 2, invoiceID: row.item.id, fileType: 1}
      let response = this.$store.dispatch('DOWNLOAD_DOC', params)
      response.then(response => {
        download(response.data, 'invoiceVAT.pdf', 'application/pdf')
      })
    },
    DownloadODT (row) {
      let params = {docType: 2, invoiceID: row.item.id, fileType: 0}
      let response = this.$store.dispatch('DOWNLOAD_DOC', params)
      response.then(response => {
        download(response.data, 'invoiceVAT.odt', 'application/vnd.oasis.opendocument.text')
      })
    },
    rowClass (item, type) {
      if (!item) return
  ...
  computed: mapGetters(['INVOICES_VAT'])
