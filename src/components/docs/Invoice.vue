<template>
  <div >
        <div v-if="INVOICES">
          <b-table hover small v-if="INVOICES" sticky-header striped id="tableStatistics"
          head-variant="light"
          :items="INVOICES"
          :fields="fields"
...
export default {
  data () {
    return {
      fields: [
        ...
        { key: 'total_tax', label: this.$i18n.t('Tax'), sortable: true, sortDirection: 'desc' },
  ...
  methods: {
    DownloadPDF (row) {
      let params = {docType: 1, invoiceID: row.item.id, fileType: 1}
    ...
    DownloadODT (row) {
      let params = {docType: 1, invoiceID: row.item.id, fileType: 0}
      let response = this.$store.dispatch('DOWNLOAD_DOC', params)
      response.then(response => {
        download(response.data, 'invoice.odt', 'application/vnd.oasis.opendocument.text')
      })
    },
    rowClass (item, type) {
      if (!item) return
